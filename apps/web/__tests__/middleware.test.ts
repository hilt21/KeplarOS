...
function makeReq(method: string, origin?: string, host = 'localhost:3000', pathname = '/api/cards'): NextRequest {
...
describe('middleware (SEC-004)', () => {
  ...
  describe('Origin host extraction', () => {
    it('extracts host from URL correctly (handles ports)', async () => {
      const res = await middleware(makeReq('POST', 'http://api.example.com:8080', 'api.example.com:8080'));
      expect(res.status).not.toBe(403);
    });
